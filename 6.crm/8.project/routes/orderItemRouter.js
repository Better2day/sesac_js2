...
router.route('/')
    .get((req, res) => {
        const cnt = db.prepare('SELECT COUNT(*) AS TOTAL FROM order_items').get();
        ...
        if (page < 1) {
            return res.redirect('/crm/order_items?page=1');
        } else if (page > totalPage) {
            return res.redirect('/crm/order_items?page=${totalPage}');
        }
        ...
        const query = db.prepare('SELECT * FROM order_items LIMIT ? OFFSET ?');
