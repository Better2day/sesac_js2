...
router.route('/')
    .get((req, res) => {
        const cnt = db.prepare('SELECT COUNT(*) AS TOTAL FROM stores').get();
        ...
        if (page < 1) {
            return res.redirect('/crm/stores?page=1');
        } else if (page > totalPage) {
            return res.redirect('/crm/stores?page=${totalPage}');
        }
        ...
        const query = db.prepare('SELECT * FROM stores LIMIT ? OFFSET ?');
