...
router.route('/')
    .get((req, res) => {
        const cnt = db.prepare('SELECT COUNT(*) AS TOTAL FROM orders').get();
        ...
        if (page < 1) {
            return res.redirect('/crm/orders?page=1');
        } else if (page > totalPage) {
            return res.redirect('/crm/orders?page=${totalPage}');
        }
        ...
        const query = db.prepare('SELECT * FROM orders LIMIT ? OFFSET ?');
